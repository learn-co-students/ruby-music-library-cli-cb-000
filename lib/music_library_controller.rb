class MusicLibraryController
  ...
  def call
    ...
    def list_artists
      Artist.all.sort_by{ |artist| artist.name }.uniq.each_with_index do |artist, i|
        puts "#{i+1}. #{artist.name}"
      end
    end
    
    def list_genres
      Genre.all.sort_by{ |genre| genre.name }.uniq.each_with_index do |genre, i|
        puts "#{i+1}. #{genre.name}"
      end
    end
    ...
    def list_songs_by_artist
      ...
      artist.songs.sort_by{|song| song.name}.each_with_index { |song, i|
        puts "#{i+1}. #{song.name} - #{song.genre.name}" } if artist != nil
    end
